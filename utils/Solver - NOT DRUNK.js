function Solver(_root){
	...
	this.canCombineObjects=function(_obj1,_obj2){
		...
		let _sign=_expression.list[_obj1.location.pos+1];
		let _sign2=_expression.list[_obj2.location.pos-1];
		if (_sign==_sign2){
			//adjacent objects
			let _operations=solver.getCorrectOperations(_expression,_scope);
			for (var i=0;i<_operations.length;i+=1){
				//correct operation
				if (_operations[i]==_sign.location.pos) return {type:Validation.SUCCESS,operations:_operations};
			}
		}else{
			//non-adjacent objects
			if ((solver.hasSign(_expression,":",_scope) || solver.hasSign(_expression,"-",_scope))){
				if (_sign.toText()=="-" || _sign.toText()==":" || _sign2.toText()=="-" || _sign2.toText()==":"){
					//non-adjacent and one of the operations is a - or :
					return {type:Validation.ERROR,text:ERROR.ORDER_OP};
				}
			}else if (solver.hasSign(_expression,"*",_scope) && solver.hasSign(_expression,"+",_scope)){
				//return false
			}else{
				return {type:Validation.SUCCESS};
			}
		...
		if (_obj1.location.pos>_scope.start && (_expression.list[_obj1.location.pos-1].toText()=="-" || _expression.list[_obj1.location.pos-1].toText()==":")){
			return {type:Validation.ERROR};
		}

		if (_sign.toText()=="*"){
			for (var i=_sign.location.pos;i<=_sign2.location.pos;i+=1){
				if (_expression.list[i].type==ObjectTypes.SIGN && _expression.list[i].toText()!="*"){
					return {type:Validation.ERROR};
				}
			}
			if (_obj1.location.pos>_scope.start && _expression.list[_obj1.location.pos-1].toText()==":"){
				return {type:Validation.ERROR};
			}

			return {type:Validation.SOFT,text:ERROR.ORDER_OP};
		}else if (_sign.toText()=="+"){
			if ((_obj1.location.pos>_scope.start && _expression.list[_obj1.location.pos-1].toText()!="+") || 
				(_obj2.location.pos<_scope.end-1 && (_expression.list[_obj2.location.pos+1].toText()=="*" || _expression.list[_obj2.location.pos+1].toText()==":"))){
				return {type:Validation.ERROR};
			}else{
				for (var i=_sign.location.pos;i<=_sign2.location.pos;i+=1){
					if (_expression.list[i].type==ObjectTypes.SIGN && _expression.list[i].toText()!="+"){
	...
	this.dragCombine=function(_obj1,_obj2){
		...
		if (_expression==_expression2){
			...
			switch (_sign.toText()){
				case "*":
					_value=_obj1.toNumber()*_obj2.toNumber();
					return {type:Validation.SUCCESS,changing:[{object:_obj1,text:String(_value)}],removing:[_sign,_obj2]};
				case "-":
					return {type:Validation.SUCCESS,changing:[{object:_obj1,text:String(_obj1.toNumber()-_obj2.toNumber())}],removing:[_sign,_obj2]};
					//return {type:Validation.HARD,text:ERROR.ERROR};
				case "+":
					_value=_obj1.toNumber()+_obj2.toNumber();
					return {type:Validation.SUCCESS,changing:[{object:_obj1,text:String(_value)}],removing:[_sign,_obj2]};
			}
			...
			if (_fraction!=_fraction2){
				...
				if (solver.canCombineObjects(_fraction,_fraction2).type!=Validation.SUCCESS){
						...
						if (root.list[_first+1].toText()!=":"){
							if (!solver.testEquivalentExpressions(_fraction.denominator.toText(),_fraction2.denominator.toText())){
								...
								for (var i=0;i<_fraction.denominator.list.length;i+=1){
									_flash.push({object:_fraction.denominator.list[i],type:Validation.HARD});
								}
								for (var i=0;i<_fraction2.denominator.list.length;i+=1){
									_flash.push({object:_fraction2.denominator.list[i],type:Validation.HARD});
								}
								_flash.push({object:root.list[_first+1],type:Validation.ERROR});
								//Add/Sub with different denom
								return {type:Validation.ERROR,flash:_flash};
							}
						}
						//Division or Add/Sub with same Denom
